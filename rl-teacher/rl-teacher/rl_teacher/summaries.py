...
def make_summary_writer(name):
    logs_path = osp.expanduser('tbdata/%s' % (name))
...
class AgentLogger(object):
    ...
    def log_simple(self, tag, simple_value, debug=False):
        ...
        #add_simple_summary(self.summary_writer, tag + '@timestep', simple_value, self._timesteps_elapsed)
        if debug:
